...
Pulls attestations from a Fide attestation registry (e.g., GitHub repo with `fide-attestation-registry` topic, or local `packages/fcp/demos/fide-attestor-template/.fide/statement-attestations/`), verifies signatures, and materializes into `fcp_raw_identifiers` and `fcp_statements`.
